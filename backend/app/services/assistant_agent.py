...
PROMPT = """You are Timebox's Assistant. Be concise and use plain text without Markdown markup. You can only read Today's stored
...
async def read_today_plan_tool() -> dict:
    ...
    try:
        ...
        plan = await asyncio.to_thread(read_today_plan)
...
async def agent_events(messages):
    model = create_model()
    ...
    with model.client:
        async with model.client:
            async with aclosing(build_agent(model).astream_events({"messages": messages}, version="v2")) as events:
                async for item in translate_events(events):
                    yield item


async def translate_events(events):
    async for event in events:
        kind = event["event"]
        if kind == "on_chat_model_stream":
            chunk = event["data"]["chunk"]
            if chunk.text:
                yield "text_delta", {"text": chunk.text}
