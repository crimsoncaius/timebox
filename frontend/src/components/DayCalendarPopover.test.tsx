...
it('contains keyboard focus and returns it after Escape and selection', async () => {
  const user = userEvent.setup()
  const onSelect = vi.fn()
  render(<DayCalendarPopover value="2026-06-01" todayIso="2026-06-15" onSelect={onSelect} />)
  const trigger = screen.getByRole('button', { name: 'Jump to date' })
  await user.tab()
  await user.keyboard('{Enter}')
  const dialog = screen.getByRole('dialog', { name: 'June 2026' })
  expect(trigger).toHaveAttribute('aria-controls', dialog.id)
  expect(screen.getByRole('button', { name: '2026-06-01' })).toHaveFocus()
