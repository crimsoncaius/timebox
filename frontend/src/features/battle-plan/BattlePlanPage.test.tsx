...
describe('BattlePlanPage', () => {
  ...
  it('creates a richly configured task and renders its relative deadline', async () => {
    ...
    await user.clear(within(composer).getByLabelText('Task type'))
    await user.type(within(composer).getByLabelText('Task type'), 'work/deep')
    await user.click(within(composer).getByRole('option', { name: /deep/i }))
