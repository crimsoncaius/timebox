import { blockIdentityText, type IdentifiableBlock } from '../../lib/blockIdentity'
...
export function recordingTimeline(preview: PlannedRecordingResult, replacement: IdentifiableBlock = preview.replacement) {
  ...
  const before: RecordingPiece[] = preview.conflicts.map(row => ({ title: blockIdentityText(row),
  ...
  after.push({ title: blockIdentityText(replacement), start, end, fresh: true })
