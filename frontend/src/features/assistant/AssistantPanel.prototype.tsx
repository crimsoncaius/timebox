...
const variants = [
  { key: 'current', name: 'Current Android', description: 'The implemented Assistant layout: inline plan card, three rows, and an expandable schedule before the answer.', tradeoff: 'Recreated from AssistantScreen.kt with sample content. This is a browser reference, not a native screenshot; Android rendering may differ.' },
...
export function CurrentAndroid(p: Props) {
  if (p.scene === 'empty') return <div className="ap-current-welcome"><h2>A little clarity for today</h2><p>Ask about your Planned Blocks. Assistant can read your plan, but can’t change it.</p>{prompts.map(prompt => <button key={prompt} onClick={() => p.prompt(prompt)}>{prompt}</button>)}</div>
  return <><div className="ap-user">{p.question}</div><div className="ap-speaker">ASSISTANT</div><section className="ap-current-plan" aria-label="Current Android plan card"><h3>Plan · 24 Sep 2026</h3><p>{p.scene === 'empty-plan' ? 0 : 5} Planned Blocks</p><p className="ap-current-meta">Read at 08:42 · Asia/Singapore</p>{p.scene === 'empty-plan' ? <p>No Planned Blocks for this date.</p> : <>{rows.slice(0, p.expanded ? 5 : 3).map(([start, end, title, type]) => <div className="ap-current-row" key={start}><time>{start}–{end}</time><div><strong>{title}</strong><small>{type}</small></div></div>)}<button onClick={() => p.setExpanded(!p.expanded)}>{p.expanded ? 'Show fewer' : 'Show all 5 blocks'}</button></>}</section><Answer empty={p.scene === 'empty-plan'} /></>
}
...
export function AssistantPanelPrototype() {
  ...
  const Component = current ? CurrentAndroid : variant.key === 'B' ? VariantB : variant.key === 'C' ? VariantC : VariantA
  return <main className="ap-lab">
    <aside className="ap-notes"><p className="ap-eyebrow">TIMEBOX / DESIGN STUDY</p><h1>{current ? <>Current Android<br />Assistant.</> : <>A calmer<br />Assistant.</>}</h1>{current && <p className="ap-baseline-note">Existing layout · recreated from Android code with sample data. Not a native screenshot.</p>}<p className="ap-intro">Three ways to make a conversation about your day feel more useful.</p><div className="ap-direction"><span>{variant.key}</span><div><h2>{variant.name}</h2><p>{variant.description}</p></div></div><p className="ap-tradeoff">{variant.tradeoff}</p>
    ...
    <section className={`ap-phone ${current ? 'ap-current' : ''} ${dark ? 'ap-dark' : ''} ${large ? 'ap-large' : ''}`} aria-label="Android Assistant preview">
      <div className="ap-status"><span>08:42</span><span>● ▰</span></div>
      {current ? <><header className="ap-header"><h2>Assistant</h2><button className="ap-current-new" aria-label="New conversation" onClick={() => reset('empty')}>{large ? '＋' : 'New conversation'}</button></header><button className="ap-current-session" onClick={() => setInfo(!info)} aria-expanded={info}>{info ? '▾' : '▸'} Temporary conversation</button></> : <header className="ap-header"><div><h2>Assistant</h2><button className="ap-session" onClick={() => setInfo(!info)} aria-expanded={info}>Temporary conversation ⓘ</button></div><button className="ap-new" aria-label="New conversation" onClick={() => reset('empty')}>＋</button></header>}
      ...
      <div className={`ap-scroll ap-variant-${variant.key}`} ref={scroll}>
        ...
        {!current && !busy && scene !== 'empty' && <button className="ap-followup" onClick={() => p.prompt('Help me think through my morning')}>Think through my morning ↗</button>}
      </div>
      <form className="ap-composer" onSubmit={e => { e.preventDefault(); send() }}><label className={current ? "ap-current-label" : "ap-sr-only"} htmlFor="ap-draft">{current ? "Message" : "Message Assistant"}</label><textarea id="ap-draft" ref={input} value={draft} onChange={e => setDraft(e.target.value)} maxLength={4000} placeholder={current ? 'Ask Assistant' : scene === 'empty' ? 'What’s on your mind?' : 'Ask a follow-up…'} rows={2} />{busy ? <button type="button" aria-label="Stop response" onClick={() => { if (timer.current) clearTimeout(timer.current); setBusy(false); setScene('interrupted') }}>{current ? 'Stop' : '■'}</button> : <button type="submit" aria-label="Send message" disabled={!draft.trim()}>{current ? 'Send' : '↑'}</button>}</form>
