...
it.each(['rail', 'sheet'] as const)('edits the whole prior-day record offline through the %s inspector and restores the changed Day', async variant => {
  ...
  expect(screen.getByLabelText('End date')).toHaveValue('2025-11-02')
  expect(screen.getByLabelText('End time')).toHaveValue('01:30')
...
it('rejects spring gaps and asks which occurrence of an autumn time before editing', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Start date'), { target: { value: '2025-03-09' } })
  fireEvent.change(screen.getByLabelText('Start time'), { target: { value: '02:30' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save changes' }))
  await waitFor(() => expect(screen.getAllByRole('alert').length).toBeGreaterThan(0))
  expect(save).not.toHaveBeenCalled()
  fireEvent.change(screen.getByLabelText('Start date'), { target: { value: '2025-11-02' } })
  fireEvent.change(screen.getByLabelText('Start time'), { target: { value: '01:40' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save changes' }))
