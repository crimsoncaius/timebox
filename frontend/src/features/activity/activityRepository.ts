...
export class ActivityRepository {
  ...
  private readJournal(): Journal {
    ...
    const outbox = value.outbox ?? (value.pending ? [value.pending] : [])
    // Preserve the entire dependent queue for recovery rather than projecting a retired command.
    if (outbox.some(command => String(command.kind) === 'describe')) {
      const previous = value.rejected
      const rejected = [...(Array.isArray(previous) ? previous : previous ? [previous] : []), ...outbox]
      const migrated = { ...value, outbox: [], pending: null, rejected }
      this.storage.setItem(storageKey, JSON.stringify(migrated))
      this.feedback = 'An obsolete activity change was rejected. Saved changes are available for recovery.'
      return migrated
    }
    return { ...value, outbox, pending: null }
  }
