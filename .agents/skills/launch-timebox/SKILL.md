...
4. For mobile, start the `Pixel_9a` AVD using the SDK in `android/local.properties`, wait for boot, run `scripts\android-gradle.ps1 :app:installDebug`, then launch `com.timebox.android/.MainActivity` with `adb`.
