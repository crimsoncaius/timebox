...
class TaskTypeRenameTest {
    ...
    fun `rename keeps draft on failure and reloads descendants on success`() = runTest {
        ...
        var viewModel: TypesViewModel? = null
        try {
            var fail = true
            ...
            val api = Proxy.newProxyInstance(TimeboxApi::class.java.classLoader, arrayOf(TimeboxApi::class.java)) { _, method, args ->
                when (method.name) {
                    ...
                    "renameTaskType" -> {
                        ...
                        rows = listOf(TaskTypeDto(1, "learning"), TaskTypeDto(2, "learning/ai"), TaskTypeDto(3, "server-only"))
                        rows[0]
                    }
            ...
            fail = false
            vm.saveRename()
            // The optimistic rename clears the sheet before the server reload finishes.
            vm.state.first { state -> state.groups.flatMap { it.items }.any { it.name == "server-only" } }
            assertEquals(listOf("learning", "learning/ai", "server-only"), vm.state.value.groups.flatMap { it.items }.map { it.name })
