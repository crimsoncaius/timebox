...
internal fun recordingTimelineModel(preview: PlannedRecordingDto): RecordingTimelineModel {
    ...
    val before = preview.conflicts.map { row ->
        RecordingPiece(row.name ?: row.taskType.name, Instant.parse(row.startAt), row.endAt?.let(Instant::parse) ?: last, running = row.endAt == null)
    }.sortedBy { it.start }
    val after = buildList {
        before.forEach { row ->
        ...
        add(RecordingPiece(preview.replacement.name ?: "Unnamed activity", start, end, fresh = true))
    }.sortedBy { it.start }
