...
private fun TrackingRound() {
 ...
 Column(Modifier.statusBarsPadding().navigationBarsPadding()) {
  Row(Modifier.fillMaxWidth().padding(horizontal=12.dp), verticalAlignment=Alignment.CenterVertically) {
   Text("PROTOTYPE 1 · ${clock(now)}", style=MaterialTheme.typography.labelSmall, modifier=Modifier.weight(1f))
   TextButton(onClick={now+=5}) { Text("+5 min") }
   TextButton(onClick={now=660; entries=emptyList(); editing=false}) { Text("Reset") }
  }
