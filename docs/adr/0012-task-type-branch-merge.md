...
The selected entry point is a rename collision on web and Android. Confirmation uses the Summary design with descendant changes expanded automatically when present; a separate merge action is outside the selected flow. Merged source identities remain internally resolvable for delayed device writes, while the source category disappears from all pickers. Prototype comparisons are preserved on `codex/prototype-task-type-merge` at `28964ed0`.
