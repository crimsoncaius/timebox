...
| 13 | Prior real-platform qualification is preserved in [Android detection](activity-tracking-android-detection.md) and [browser detection](activity-tracking-browser-detection.md). It includes API 33/36, usage access, reboot/Doze, notifications, Chromium permissions, natural idle sensing, page closure/renderer crash recovery and unsupported Firefox. Real Chromium hidden/freeze/discard transitions were attempted but not verified; those paths have deterministic coverage only. These longer qualification experiments were not all repeated in #158. |
