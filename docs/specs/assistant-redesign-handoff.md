...
Approved implementation amendment (2026-09-22): a complete, otherwise valid snapshot selector may omit its newline only when the provider confirms normal successful completion and there is no answer text. The newline remains mandatory before answer text. EOF, cancellation, length truncation, malformed JSON, extra/duplicate fields and unauthorized references do not qualify. This amendment addresses observed live card-only outputs without adding model calls or retries.
